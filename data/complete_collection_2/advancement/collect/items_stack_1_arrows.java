...
public class items_stack_1_arrows {
    ...
    public static ArrayList<String> findItem(File s)  throws java.io.FileNotFoundException, java.util.NoSuchElementException{
        ...
        while (sc.hasNextLine()){
            ...
            if (index >= 0){
                sc.nextLine();
                pat = sc.nextLine();
                if (pat.contains("potion_contents")){
    ...
    for (int i = 0; i < finals.size(); i++){
        for (String pat : patterns){
                    System.out.println("\""+pat.substring(37, pat.length()-1) +"_"+ finals.get(i) + "\": {");
                    ...
                    System.out.println("\"minecraft:container\": [");
                        for (int j = 0; j < 26; j++){
                            ...
                            System.out.println("{\"slot\": " + j + ", \"item\": {\"id\": \"minecraft:" + finals.get(i) + "\", \"predicates\":{" + pat + "},\"count\":64}},");
                        }                                                                                                                       
                            System.out.println("{\"slot\": 26, \"item\": {\"id\": \"minecraft:" + finals.get(i) + "\", \"predicates\":{" + pat + "},\"count\":64}}");
                    System.out.println("]");
                    System.out.println("}");
                    System.out.println("}");
                    System.out.println("]");
